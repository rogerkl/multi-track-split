...
- `export.rs` — `render_multitrack` (active tracks only, zero outside each track's range), `render_mixdown` (unclamped), `write_audio` which picks integer FLAC (flacenc, ≤ 8 channels) or WAV from the extension at the source bit depth (16–24), and `write_float_wav` (32-bit IEEE float). Mixdowns go to float WAV by default and are only clamped when written as FLAC; the hand-written RIFF writer uses WAVE_FORMAT_EXTENSIBLE above 2 channels. `export_all_multitrack` / `export_all_mixdowns` batch every song into a folder.
