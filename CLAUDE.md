...
- `audio.rs` — decodes WAV/FLAC via symphonia into `AudioData`: the **entire recording in memory** as **planar** f32 (`tracks[channel][frame]`, normalized to [-1, 1]) plus per-channel waveform peak bins (`PEAK_BIN` = 512). `load_many` accepts one interleaved file or several mono/stereo files (natural-sorted by name, same sample rate required, padded with silence to the longest); `sources`/`source_channels` record where each track came from and drive `default_track_names`. Also `format_time`/`parse_time` (`[[h:]mm:]ss[.mmm]`). Shared as `Arc<AudioData>`.
- `project.rs` — the data model and its JSON sidecar file (`<stem>.mtsplit.json` next to a single recording, `<dir>.mtsplit.json` for a set of files; auto-loaded when the recording is opened). `audio_files` lists the source files in track order; the legacy single `audio_file` field still loads via `Project::files()`. `Song { start, end, tracks: Vec<TrackSettings> }`; `TrackSettings { active, start, end (absolute-frame overrides), volume, pan, mute, solo }`. `Song::track_range(ch)` is the single source of truth for a track's effective range (song range narrowed by overrides, `None` when inactive).
...
- `export.rs` — `render_multitrack` (active tracks only, zero outside each track's range), `render_mixdown` (unclamped), `write_audio` which picks integer FLAC (flacenc, ≤ 8 channels) or WAV from the extension at the source bit depth (16–24), and `write_float_wav` (32-bit IEEE float). `MixFormat` (stored in the project) selects float WAV or a normalized 16/24-bit WAV/FLAC (mix scaled to a `NORMALIZE_TARGET_DB` = -1 dBFS peak); the hand-written RIFF writer uses WAVE_FORMAT_EXTENSIBLE above 2 channels. `StemFormat` (stored in the project) picks interleaved vs one mono file per track (`export_stem_tracks`, files named `Tnn - <track name>` inside a `NN - Title` folder). `export_all_multitrack` / `export_all_mixdowns` batch every song into a folder.
