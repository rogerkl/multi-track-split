...
5. **Save project** writes the JSON sidecar. **Export mix…** writes the song's stereo mix in the format chosen in the *Mix as* list: 32-bit float WAV (nothing scaled, so the mix never clips no matter how hot the faders are), or a normalized 16- or 24-bit WAV/FLAC, where the whole mix is scaled so its peak sits at -1 dBFS. The choice is stored in the project. **Export multitrack…** writes the stems of the song: only the tracks it uses, each silent outside its own in/out range, so a track that joins 20 seconds in is silent for its first 20 seconds. The *Stems as* list picks the layout: one interleaved WAV or FLAC (interleaved FLAC is only offered for recordings of up to 8 tracks, FLAC's limit), or one mono file per track (`T03 - Bass.wav`, numbered by tape track) inside a `NN - Title` folder. **Export all multitrack…** does this for every song into a chosen folder (`NN - Title.flac`, or one `NN - Title/` folder per song; interleaved FLAC falls back to WAV above 8 tracks); **Export all mixes…** does the same for the mixes (`NN - Title (mix).wav` / `.flac`) in the chosen mix format.
