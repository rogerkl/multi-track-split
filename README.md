...
5. **Save project** writes the JSON sidecar. **Export mix…** writes the song's stereo mix as a 32-bit float WAV, so the mix never clips no matter how hot the faders are (choose `.flac` for an integer file at the recording's bit depth; that one is clamped). **Export multitrack…** writes a FLAC/WAV containing only the tracks the song uses, in tape order, each silent outside its own in/out range, so a track that joins 20 seconds in is silent for its first 20 seconds. **Export all multitrack…** writes one multitrack file per song into a folder (`NN - Title.flac`, WAV when a song uses more than 8 tracks, FLAC's limit); **Export all mixes…** does the same for the float WAV mixes (`NN - Title (mix).wav`).
